    ...
    for month, monthly_data in data.groupby(data.index):
        ...
        for i in range(5):
            for j in range(5):
                ...
                BP_list = list(sort_BP_ME[j*length:(j+1)*length]["Code"])
                stock_list = [x for x in ME_list if x in BP_list]
...
#%%
FamaFrench.to_csv(path + "\\FamaFrench.csv")